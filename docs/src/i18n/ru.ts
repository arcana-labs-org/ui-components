...
export const ru: Messages = {
  ...
  gettingStarted: {
    ...
    deps: {
      title: "Зависимости",
      p1: "У этой библиотеки <b>нет зависимостей времени выполнения</b>: установка ничего не добавляет в вашу сборку. Всё, что ниже, — <b>peer</b>-зависимости: их предоставляет ваше приложение, а здесь они объявлены, чтобы вы знали, что нужно каждому компоненту.",
      p2: "Каждая peer-зависимость <b>необязательная</b>: ставьте только тот фреймворк, который используете, плюс дополнения для нужных возможностей. При установке ничего не сломается, если что-то пропустить — требует её тот компонент, которому она нужна."
    },
    maska: {
      title: "Маски",
      p1: "Маска ввода доступна во <b>всех четырёх фреймворках</b> через пакет <c>maska</c> — это <b>необязательная peer-зависимость</b>, установите её, когда отрисовываете <c>ArcanaInputMask</c>. Только у Vue есть дополнительный шаг: один раз глобально зарегистрировать директиву <c>v-maska</c>.",
  ...
  dependencies: {
    ...
    note: "Ничего отсюда не скачивается без вашего запроса — необязательные peer важны, только если вы отрисовываете использующий их компонент."
  },
  ...
  components: {
    ...
    radio: {
      blurb: "Одиночный радио-переключатель: нативный <c>&lt;input type=\"radio\"&gt;</c> с кружком дизайн-системы и необязательным лейблом. Группируйте несколько через общий <c>name</c> и <c>v-model</c>."
    },
    radioCardGroup: {
    ...
    dropdown: {
      blurb: "Выпадающее меню shadcn, телепортируемое в <c>&lt;body&gt;</c>, авто-позиционируемое и закрывающееся по клику снаружи, Escape или выбору. Второй пример показывает рецепт <c>quick date filter</c> (Сегодня / Вчера / Последние N дней + Больше опций)."
    },
    dropdownItem: {
    ...
    tooltip: {
      blurb: "Короткая текстовая подсказка, открывающаяся при наведении или фокусе на триггер, со стрелкой и позиционированием через <c>side</c>/<c>align</c> (авто-переворот). Текст передаётся в <c>label</c>."
    },
    contextMenu: {
  ...
  demos: {
    ...
    qdfToday: "Сегодня",
    qdfYesterday: "Вчера",
    qdfLast7: "Последние 7 дней",
    qdfLast30: "Последние 30 дней",
