...
export const de: Messages = {
  ...
  components: {
    ...
    input: {
      blurb: "Ein natives <c>&lt;input&gt;</c> mit shadcn-Styling und einem zahlenbewussten <c>v-model</c>; Standard-HTML-Attribute werden direkt durchgereicht."
    },
    select: {
    ...
    checkbox: {
      blurb: "Eine binäre Checkbox, die ein echtes natives <c>&lt;input type=\"checkbox\"&gt;</c> mit <c>indeterminate</c>-Zustand umschließt; für Einstellungen nimm <c>ShadcnSwitch</c>."
    },
    switch: {
    ...
    dialog: {
      blurb: "Ein Modal im shadcn-Stil mit ref-basierter <c>show()</c>/<c>hide()</c>-API — teleportiert ins <c>&lt;body&gt;</c>, fängt den Fokus und schließt bei Escape."
    },
    inputMask: {
    ...
    datePicker: {
      blurb: "Ein Datumsfeld im shadcn-Stil, das ein maskiertes <c>DD/MM/AAAA</c>-Eingabefeld mit einem Kalender-Popover verbindet; das <c>v-model</c> ist ein ISO-String <c>YYYY-MM-DD</c>."
    },
    inputCurrency: {
    ...
    labeledButton: {
      blurb: "Der Basis-Button mit <c>label</c>, optionalem <c>icon</c> und einem <c>loading</c>-Spinner-Zustand; das <c>shadcn</c>-Flag bildet alte Farben auf semantische Varianten ab."
    },
    accordion: {
      blurb: "Der Container für einklappbare <c>ShadcnAccordionItem</c>s, der ein <c>v-model</c> für Einzel- oder Mehrfach-Öffnen-Modus bindet."
    },
    accordionItem: {
      blurb: "Ein einzelnes einklappbares Panel innerhalb eines <c>ShadcnAccordion</c>, identifiziert durch einen <c>name</c>, mit <c>title</c>-Header und Slot-Inhalt."
    },
    dropdown: {
      blurb: "Ein Dropdown-Menü im shadcn-Stil, das ins <c>&lt;body&gt;</c> teleportiert, sich automatisch positioniert und bei Außenklick, Escape oder Auswahl schließt."
    },
    dropdownItem: {
      blurb: "Eine Zeile innerhalb eines <c>ShadcnDropdown</c> — optionales <c>icon</c>, Label und <c>suffix</c> — färbbar, mit einer <c>divided</c>-Trennlinie für destruktive Aktionen."
    },
    table: {
      blurb: "Eine statische Tabelle im shadcn-Stil für Arrays im Speicher; Spalten deklarieren <c>{ key, label, width?, align?, valueGetter? }</c>, mit Zell- und Footer-Slots."
    },
    specSheet: {
    ...
    specSheetSection: {
      blurb: "Ein Abschnitt innerhalb eines <c>ShadcnSpecSheet</c> — Akzent-<c>icon</c>, <c>title</c> und <c>sectionNum</c> über einem <c>columns</c>-Raster aus Feldern."
    },
    specSheetField: {
    ...
    settingsListGroup: {
      blurb: "Ein betitelter, optional <c>collapsible</c>-Abschnitt innerhalb eines <c>ShadcnSettingsList</c>, mit Icon, <c>sectionNum</c> und <c>meta</c>."
    },
    settingsListItem: {
