...
export const ja: Messages = {
  ...
  gettingStarted: {
    ...
    deps: {
      title: "依存関係",
      p1: "このライブラリに<b>ランタイム依存はありません</b>。導入してもバンドルには何も増えません。以下はすべて <b>peer</b> で、アプリ側が用意します。各コンポーネントが何を必要とするか分かるように、ここで宣言しています。",
      p2: "peer はすべて<b>オプション</b>です。実際に使うフレームワークと、描画する機能に必要な追加パッケージだけを入れてください。入れ忘れてもインストールは失敗しません — 必要とするのは、それを使うコンポーネントです。"
    },
    maska: {
      title: "マスク",
  ...
  dependencies: {
    ...
    note: "ここにあるものは、求めない限りダウンロードされません — 任意の peer は、それを使うコンポーネントを描画する場合にのみ関係します。"
  },
