...
describe("ArcanaGridEmptyState", () => {
    ...
    it("mostra o slot (grid) quando total > 0", async () => {
        const w = mount(ArcanaGridEmptyState, {
        ...
        expect(w.find(".arcana-onboarding").exists()).toBe(false)
    })
    ...
    it("mostra o painel de onboarding quando vazio e não filtrado (após loaded)", async () => {
        ...
        expect(w.find(".arcana-onboarding").exists()).toBe(true)
    ...
    it("emite @action quando clica na CTA do painel", async () => {
        ...
        await w.find(".arcana-onboarding__cta").trigger("click")
