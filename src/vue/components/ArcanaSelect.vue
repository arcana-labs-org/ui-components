<template>
    ...
    >
        ...
        <button
            ...
            :class="{
                'arcana-select__trigger--open': isOpen,
                'arcana-select__trigger--has-clear': canClear,
            }"
            :disabled="disabled"
        ...
        <!--
            ...
            com `overflow: hidden` ou z-index restritivo (ex: dentro do ArcanaDialog).
            Posição calculada com base no rect do trigger (position: fixed).
        -->
        <Teleport to="body">
            <Transition name="arcana-select-fade">
                <div
                    v-if="isOpen"
                    ref="panelRef"
...
/**
 * `<ArcanaSelect>` — Select custom arcana-style, totalmente implementado em Vue/CSS.
...
export default {
    ...
    computed: {
        rootClasses(): string {
            const cls = ['arcana-select', `arcana-select--${this.size}`]
            if (this.disabled) cls.push('arcana-select--disabled')
            if (this.isOpen) cls.push('arcana-select--open')
            return cls.join(' ')
        },
    ...
    methods: {
        ...
        scrollHighlightedIntoView() {
            this.$nextTick(() => {
                ...
                if (!panel) return
                const item = panel.querySelector<HTMLElement>('.arcana-select__item.is-highlighted')
