<template>
    <section :class="['shadcn-spec-sheet__section', { 'shadcn-spec-sheet__section--compact': compact }]">
        <header v-if="hasHeader" class="shadcn-spec-sheet__section-head">
            ...
            <div class="shadcn-spec-sheet__section-head-right">
                <span v-if="sectionNum" class="shadcn-spec-sheet__section-num">{{ sectionNum }}</span>
                <!--
        ...
        <!--
            Grid configurável (1/2/3/4 colunas). Cada child deve ser um `<ShadcnSpecSheetField>`.
        ...
        <div
            :class="[
                ...
                { 'shadcn-spec-sheet__grid--no-row-dividers': noRowDividers },
            ]"
        >
            <slot />
        </div>
...
/**
 * `<ShadcnSpecSheetSection>` — section interna do `<ShadcnSpecSheet>`.
 *
 * Cada section tem ícone boxed colorido opcional + título + section-num opcional +
 * actions à direita + grid 2-col com fields (`<ShadcnSpecSheetField>`).
 *
