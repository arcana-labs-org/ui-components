<template>
    <div class="shadcn-dropdown" ref="containerRef">
        <span class="shadcn-dropdown__trigger" @click.stop="toggle">
            <slot name="trigger" :open="isOpen" :toggle="toggle"/>
        </span>

        <Teleport to="body">
            <transition name="shadcn-dropdown">
                <div
                    v-if="isOpen"
                    class="shadcn-dropdown__menu"
                    :class="{ 'shadcn-dropdown__menu--comfortable': size === 'comfortable' }"
                    :style="menuStyle"
                    ref="menuRef"
                    role="menu"
                    @click="onMenuClick"
...
/**
 * `<ShadcnDropdown>` — dropdown menu no padrão shadcn (palette zinc, radius 8px,
 * sombra suave, animação fade+slide). Substitui `<el-dropdown>` do Element Plus
 * pra manter a linguagem visual consistente com `ShadcnDialog`/`ShadcnButton`.
 *
 * Como usar:
 * ```vue
 ...
 *     </template>
 *
 *     <ShadcnDropdownItem icon="fa-solid fa-pen" @click="rename">Renomear</ShadcnDropdownItem>
 *     <ShadcnDropdownItem icon="fa-solid fa-trash" variant="danger" @click="del">Deletar</ShadcnDropdownItem>
 * </ShadcnDropdown>
 * ```
 *
 * Decisões:
 ...
 * - **Close on**: click fora, ESC, ou click em `ShadcnDropdownItem` (item
 *   dispatcha `shadcn-dropdown-close` que sobe via bubble e o menu fecha).
...
export default {
    ...
    props: {
        ...
        /**
         * Densidade do menu. Propagada pros `ShadcnDropdownItem` filhos via
    ...
    mounted() {
        // Listener pra evento customizado emitido por ShadcnDropdownItem ao
        ...
        document.addEventListener('shadcn-dropdown-close', closeOnItem)
    },

    unmounted() {
        this.close()
        if ((this as any)._closeOnItem) {
            document.removeEventListener('shadcn-dropdown-close', (this as any)._closeOnItem)
        }
    },
} as Component
</script>
...
.shadcn-dropdown__trigger {
    display: inline-block;
}
</style>

<style>
/* NÃO escopado de propósito: o menu é teleportado pro body, fora do escopo
   do componente. Use namespace `.shadcn-dropdown__*` pra evitar colisão. */
.shadcn-dropdown__menu {
    position: fixed;
    min-width: 180px;
    max-width: 320px;
    background: white;
...
/* Densidade folgada (size="comfortable"): mais respiro no menu. O padding
   dos próprios itens é tratado no ShadcnDropdownItem via inject. */
.shadcn-dropdown__menu--comfortable {
    min-width: 200px;
    padding: 6px;
}

/* Transição fade + slide-down — espelha animação de dropdowns shadcn-ui */
.shadcn-dropdown-enter-active,
.shadcn-dropdown-leave-active {
    transition: opacity 0.12s ease, transform 0.12s ease;
}
.shadcn-dropdown-enter-from,
.shadcn-dropdown-leave-to {
    opacity: 0;
    transform: translateY(-4px) scale(0.98);
}
.shadcn-dropdown-enter-to,
.shadcn-dropdown-leave-from {
    opacity: 1;
