<template>
    <div class="arcana-tabs" :class="rootClasses">
        <!--
        ...
        -->
        <div class="arcana-tabs__list" role="tablist" :aria-label="ariaLabel || undefined" @keydown="onKeydown">
            <!--
            ...
            <template v-for="item in renderableItems" :key="item.key">
                <div v-if="item.type === 'header'" class="arcana-tabs__group-header" role="presentation">{{ item.label
                    }}</div>
                <el-tooltip v-else :content="item.tab.label" :placement="tooltipPlacement || 'right'" :show-after="80"
                    ...
                    <button ref="triggerRefs" type="button" class="arcana-tabs__trigger" :class="{
                        ...
                        <i v-if="item.tab.icon" :class="['arcana-tabs__trigger-icon', item.tab.icon]"
                            :style="item.tab.iconColor ? { color: item.tab.iconColor } : undefined"></i>
                        <span class="arcana-tabs__trigger-label">{{ item.tab.label }}</span>
                        <span v-if="item.tab.badge != null" class="arcana-tabs__trigger-badge">{{ item.tab.badge
                            }}</span>
                    </button>
                </el-tooltip>
            </template>
        ...
        <template v-if="keepAlive">
            <template v-for="tab in normalizedTabs" :key="String(tab.name)">
                <div v-if="wasActivated(tab)" v-show="isActive(tab)" class="arcana-tabs__panel"
        ...
        <div v-else-if="activeTab" class="arcana-tabs__panel" role="tabpanel" :id="panelId(activeTab)"
...
/**
 * `<ArcanaTabs>` — tabs arcana-style (palette zinc), implementação custom (sem `<el-tabs>`).
 *
 * Variants visuais:
 ...
 *   `--arcana-tabs-accent`, `--arcana-tabs-soft-bg`, `--arcana-tabs-soft-border`.
 ...
 *
 *     <ArcanaTabs v-model="area" variant="sidebar-soft" :tabs="[
 ...
 *     </ArcanaTabs>
 *
 *     <ArcanaTabs v-model="sub" variant="segmented" :tabs="[
...
export default {
    ...
    props: {
        ...
        /**
         * Remove qualquer margin do `.arcana-tabs__list`. Útil quando o `ArcanaTabs` está
         * ANINHADO dentro de outro cujo wrapper aplica margin via `:deep(.arcana-tabs__list)`
