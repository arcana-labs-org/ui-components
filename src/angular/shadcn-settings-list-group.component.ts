...
/**
 * `ShadcnSettingsListGroupComponent` — Angular port do SFC Vue `ShadcnSettingsListGroup`.
 *
 * Attribute selector num `<section>` (`<section arcanaShadcnSettingsListGroup>`): section
 * interna do `ShadcnSettingsList` com header (ícone boxed + título + meta) e opção
 * `collapsible` (header vira botão + chevron). Reproduz `.shadcn-settings-list__group`
...
@Component({
  selector: "section[arcanaShadcnSettingsListGroup]",
  standalone: true,
  ...
  template: `
    @if (title || titleTemplate) {
      @if (collapsible) {
        ...
        <header class="shadcn-settings-list__group-head">
          <ng-container [ngTemplateOutlet]="headInner"></ng-container>
        </header>
      }
    }
    ...
    <ng-template #headInner>
      <div class="shadcn-settings-list__group-head-left">
        ...
        <div>
          @if (sectionNum) {
            <div class="shadcn-settings-list__group-num">{{ sectionNum }}</div>
          }
      ...
      <div class="shadcn-settings-list__group-head-right">
        @if (meta || metaTemplate) {
          <div class="shadcn-settings-list__group-meta">
            @if (metaTemplate) {
...
export class ShadcnSettingsListGroupComponent implements OnInit {
  @Input() title = "";
  @Input() sectionNum = "";
  @Input() meta = "";
  @Input() icon = "";
  ...
  get rootClass(): string {
    return [
      ...
      this.compact ? "shadcn-settings-list__group--compact" : "",
      this.collapsible ? "shadcn-settings-list__group--collapsible" : "",
      this.collapsible && this.isCollapsed ? "shadcn-settings-list__group--collapsed" : ""
    ].filter(Boolean).join(" ");
  }
