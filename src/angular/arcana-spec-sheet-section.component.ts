...
/**
 * `ArcanaSpecSheetSectionComponent` — Angular port do SFC Vue `ArcanaSpecSheetSection`.
 *
 * Attribute selector num `<section>` (`<section arcanaSpecSheetSection>`): section
 * interna do `ArcanaSpecSheet` com ícone boxed colorido opcional + título + section-num +
 * actions + grid N-col. Reproduz `.arcana-spec-sheet__section` (+ `--compact`), `__section-head`/
...
@Component({
  selector: "section[arcanaSpecSheetSection]",
  standalone: true,
...
export class ArcanaSpecSheetSectionComponent {
  @Input() title = "";
  @Input() sectionNum = "";
  @Input() icon = "";
  ...
  get rootClass(): string {
    return [
      "arcana-spec-sheet__section",
      this.compact ? "arcana-spec-sheet__section--compact" : ""
    ].filter(Boolean).join(" ");
  }
  ...
  get gridClass(): string {
    return [
      ...
      `arcana-spec-sheet__grid--cols-${this.columns}`,
      this.noRowDividers ? "arcana-spec-sheet__grid--no-row-dividers" : ""
    ].filter(Boolean).join(" ");
  }
