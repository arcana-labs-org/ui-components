...
/**
 * `InputCurrencyComponent` — Angular port do SFC Vue `InputCurrency`. Input de moeda BRL.
 *
 ...
 *   (dígitos preenchem dos centavos pra cima), mantendo markup/classes (`icur-shadcn-field`/
 *   `icur-shadcn-input` no modo shadcn; `input-group`/`form-control` no Bootstrap).
...
@Component({
  ...
  template: `
    @if (prependTemplate) {
      ...
      @if (showIcon && shadcn) {
        <span class="icur-shadcn-field__icon"><i [class]="icon"></i></span>
      }
    }
...
export class InputCurrencyComponent implements OnChanges {
  ...
  get inputClasses(): string {
    return [
      this.shadcn ? "icur-shadcn-input" : "form-control",
  ...
  get wrapClasses(): string {
    if (this.shadcn) {
      return ["icur-shadcn-field", this.isDisabled ? "is-disabled" : ""].filter(Boolean).join(" ");
    }
