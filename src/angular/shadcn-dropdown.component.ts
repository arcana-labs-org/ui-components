...
/**
 * `ShadcnDropdownComponent` — Angular port do SFC Vue `ShadcnDropdown`.
 *
 * Attribute selector num `<div>` (`<div arcanaShadcnDropdown>`): container `.shadcn-dropdown`
 * com trigger + menu shadcn (palette zinc). Reproduz `.shadcn-dropdown__trigger`, o
 * `.shadcn-dropdown__menu` (+ `--comfortable`) TELEPORTADO pro `<body>` e o posicionamento
 ...
 * seu root pro `document.body` (como o `ShadcnSelect`). Os `ShadcnDropdownItem` projetados
 * (`<ng-content>`) seguem na árvore de CD e injetam ESTE componente via
 * `inject(ShadcnDropdownComponent)` — substitui o `provide('shadcnDropdownSize')` + o
 * CustomEvent `shadcn-dropdown-close` do SFC (o item chama `close()` direto).
 *
 * Vue → Angular:
...
@Component({
  selector: "div[arcanaShadcnDropdown]",
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [NgTemplateOutlet],
  host: { "class": "shadcn-dropdown" },
  template: `
    <span class="shadcn-dropdown__trigger" (click)="onTriggerClick($event)">
      @if (triggerTemplate) {
        <ng-container
...
export class ShadcnDropdownComponent implements OnDestroy {
  ...
  @Input() placement: ShadcnDropdownPlacement = "bottom-end";
  @Input() offset = 4;
  @Input() disabled = false;
  @Input() size: ShadcnDropdownSize = "default";
