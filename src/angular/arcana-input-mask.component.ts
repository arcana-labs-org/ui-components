...
/**
 * `ArcanaInputMaskComponent` — Angular port do SFC Vue `ArcanaInputMask`.
 *
 * Attribute selector num `<input>` (`<input arcanaInputMask>`): input mascarado
 * shadcn (`class="arcana-input"` + `--${size}`/`--disabled`), idêntico ao Vue/React.
 *
 ...
 */
@Component({
  selector: "input[arcanaInputMask]",
  standalone: true,
...
export class ArcanaInputMaskComponent implements OnChanges {
