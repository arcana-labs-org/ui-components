...
/**
 * `ShadcnRadioCardGroupComponent` — Angular port do SFC Vue `ShadcnRadioCardGroup`.
 *
 * Attribute selector num `<div>` (`<div arcanaShadcnRadioCardGroup>`): reproduz
 * `.shadcn-radio-card-group` (+ `--inline`/`--grid`/`--radio-end`), cada
 * `<label class="shadcn-radio-card">` (+ `is-selected`/`is-disabled`), o `__input` nativo,
...
@Component({
  selector: "div[arcanaShadcnRadioCardGroup]",
  standalone: true,
    ...
    @for (opt of normalizedOptions; track $index) {
      ...
      >
        ...
        <span class="shadcn-radio-card__content">
          ...
          @if (opt.description) {
            <span class="shadcn-radio-card__desc">{{ opt.description }}</span>
          }
        </span>

        @if (opt.badge) {
          <span class="shadcn-radio-card__badge">{{ opt.badge }}</span>
        }
      </label>
    }
  `
})
export class ShadcnRadioCardGroupComponent {
  ...
  get rootClass(): string {
    return [
      ...
      this.inline && !this.columns ? "shadcn-radio-card-group--inline" : "",
      this.columns > 0 ? "shadcn-radio-card-group--grid" : "",
      this.radioPosition === "end" ? "shadcn-radio-card-group--radio-end" : "",
      this.className
    ].filter(Boolean).join(" ");
  }
