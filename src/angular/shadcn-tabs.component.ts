...
/**
 * `ShadcnTabsComponent` — Angular port do SFC Vue `ShadcnTabs`.
 *
 * Attribute selector num `<div>` (`<div arcanaShadcnTabs>`): reproduz `.shadcn-tabs`
 * (+ `--${variant}`, `--${orientation}`, `--flush`), `.shadcn-tabs__list`,
...
@Component({
  ...
  template: `
    <div class="shadcn-tabs__list" role="tablist" [attr.aria-label]="ariaLabel || null" (keydown)="onKeydown($event)">
...
export class ShadcnTabsComponent implements OnInit, OnChanges {
  @Input({ required: true }) value!: string | number;
  @Input({ required: true }) tabs!: ShadcnTabItem[];
  @Input() variant: ShadcnTabsVariant = "pills";
  ...
  get normalizedTabs(): ShadcnTabItem[] {
    return this.tabs ?? [];
  }

  get activeTab(): ShadcnTabItem | null {
  ...
  panelTemplate(tab: ShadcnTabItem): TemplateRef<unknown> | null {
    return this.panels?.find((p) => String(p.name) === String(tab.name))?.templateRef ?? null;
  }

  select(tab: ShadcnTabItem): void {
  ...
  private focusTrigger(tab: ShadcnTabItem): void {
