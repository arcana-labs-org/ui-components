...
/**
 * `ArcanaDialogComponent` — Angular port do SFC Vue `ArcanaDialog`.
 *
 * Attribute selector num `<div>` (`<div arcanaDialog>`): modal arcana-style com API
 * imperativa (`show()` / `hide()`), espelhando a API ref-based do SFC (`this.$refs.dialog
 * .show()`). Emite o MESMO markup e as MESMAS classes shadcn (`.arcana-dialog-overlay`,
 * `.arcana-dialog-content`, `__header`/`__body`/`__footer`) do Vue/React.
 *
 * Portal (idêntico ao `ArcanaSelect`/`MultiSelectPopover`): o overlay vive num
    ...
    <ng-template #dialogTpl>
      ...
      >
        <div
          ...
          [class.arcana-dialog-content--full-height]="fullHeight"
          [class]="contentClasses"
          role="dialog"
          aria-modal="true"
          [attr.aria-labelledby]="title ? 'arcana-dialog-title' : null"
          [attr.aria-describedby]="description ? 'arcana-dialog-desc' : null"
          [style.maxWidth]="resolvedMaxWidth"
          [style.zIndex]="currentZIndex + 1"
        >
          @if (showHeader) {
            <header class="arcana-dialog-header">
              @if (headerTemplate) {
                <ng-container [ngTemplateOutlet]="headerTemplate"></ng-container>
              } @else {
                <h2 id="arcana-dialog-title" class="arcana-dialog-title">{{ title }}</h2>
                @if (description) {
                  <p id="arcana-dialog-desc" class="arcana-dialog-description">{{ description }}</p>
                }
              }
              @if (closeable) {
                <button
          ...
          <div
            ...
            [class.arcana-dialog-body--no-padding]="noBodyPadding"
            [class.arcana-dialog-body--no-scroll]="!bodyScrollable"
          >
...
export class ArcanaDialogComponent implements OnDestroy {
  ...
  get contentClasses(): string {
    return ["arcana-dialog-content", this.contentClass].filter(Boolean).join(" ");
  }
  ...
  private focusFirstElement(): void {
    const root = this.overlayEl?.querySelector<HTMLElement>(".arcana-dialog-content");
    if (!root) return;
