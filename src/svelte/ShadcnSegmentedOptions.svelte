<script lang="ts">
  /**
   * `<ShadcnSegmentedOptions>` — Svelte 5 port. Segmented control de N opções. Reproduz
...
>
  {#each normalizedOptions as opt (String(opt.value))}
    <button
      type="button"
      class={["shadcn-segmented-options__option", opt.value === value ? "is-active" : ""]
        .filter(Boolean)
        .join(" ")}
      role="radio"
      ...
      onclick={() => select(opt)}
    >
      {#if radio}<span class="shadcn-segmented-options__radio" aria-hidden="true"></span>{/if}
      {#if opt.icon}<i class={`shadcn-segmented-options__icon ${opt.icon}`}></i>{/if}
      <span>{opt.label}</span>
    </button>
  {/each}
