<script lang="ts">
  /**
   * `<ShadcnSpecSheet>` — Svelte 5 port do SFC Vue. Display read-only de dados em formato
   * editorial/spec sheet. Markup/classes `shadcn-spec-sheet*` idênticos ao SFC.
   *
  ...
  const rootClasses = $derived(
    ["shadcn-spec-sheet", flat ? "shadcn-spec-sheet--flat" : ""].filter(Boolean).join(" ")
  );
</script>
...
<article class={rootClasses}>
  ...
  {#if footer}
    <footer class="shadcn-spec-sheet__footer">{@render footer()}</footer>
  {/if}
</article>
