<script lang="ts">
  ...
  /**
   * `<ShadcnButton>` — Svelte 5 port do SFC Vue. Emite o MESMO markup/classes
   * (`shadcn-button`, `shadcn-button--${variant}`, `is-disabled`), reusando o CSS
  ...
  const classes = $derived(
    ["shadcn-button", `shadcn-button--${variant}`, disabled ? "is-disabled" : "", className]
      .filter(Boolean)
      .join(" ")
  );
</script>
