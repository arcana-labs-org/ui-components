<script lang="ts">
  /**
   * `<ArcanaDialog>` — Svelte 5 port do SFC Vue. Modal arcana-style com API imperativa
   ...
   *   `bind:this` no componente (mesma abordagem imperativa do ArcanaTable/datatable)
  ...
  type ArcanaDialogSize = "sm" | "md" | "lg" | "xl" | "full" | number;
  ...
  const contentClasses = $derived(
    [
      "arcana-dialog-content",
      fullHeight ? "arcana-dialog-content--full-height" : "",
      contentClass,
    ]
      .filter(Boolean)
      .join(" ")
  ...
  const bodyClasses = $derived(
    [
      "arcana-dialog-body",
      noBodyPadding ? "arcana-dialog-body--no-padding" : "",
      !bodyScrollable ? "arcana-dialog-body--no-scroll" : "",
    ]
      .filter(Boolean)
      .join(" ")
  );
  const footerClasses = $derived(
    ["arcana-dialog-footer", flatFooter ? "arcana-dialog-footer--flat" : ""]
      .filter(Boolean)
      .join(" ")
  );
</script>
