...
<script lang="ts">
  /**
   * `<InputCurrency>` — Svelte 5 port. Input de moeda BRL.
   *
   ...
   * (`icur-shadcn-field`/`icur-shadcn-input` no modo shadcn; `input-group`/`form-control`
  ...
  const inputClasses = $derived(
    [
      shadcn ? "icur-shadcn-input" : "form-control",
  ...
  const wrapClasses = $derived(
    shadcn
      ? ["icur-shadcn-field", isDisabled ? "is-disabled" : ""].filter(Boolean).join(" ")
      : showIcon
        ? "input-group"
        : ""
  );
