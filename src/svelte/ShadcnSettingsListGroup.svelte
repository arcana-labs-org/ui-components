...
<script lang="ts">
  /**
   * `<ShadcnSettingsListGroup>` — Svelte 5 port do SFC Vue. Section interna do
   * `<ShadcnSettingsList>` com header (ícone boxed opcional + title + meta opcional).
   *
  ...
  const sectionClasses = $derived(
    [
      ...
      compact ? "shadcn-settings-list__group--compact" : "",
      collapsible ? "shadcn-settings-list__group--collapsible" : "",
      collapsible && isCollapsed ? "shadcn-settings-list__group--collapsed" : "",
    ]
      .filter(Boolean)
      .join(" ")
  );
