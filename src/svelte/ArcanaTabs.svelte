...
<script lang="ts">
  ...
  /**
   * `<ArcanaTabs>` — Svelte 5 port do SFC Vue. Reproduz o mesmo markup/classes:
  ...
  const triggerId = (tab: ArcanaTabItem) => `arcana-tabs-${uid}-trigger-${String(tab.name)}`;
  const panelId = (tab: ArcanaTabItem) => `arcana-tabs-${uid}-panel-${String(tab.name)}`;
  ...
  function select(tab: ArcanaTabItem) {
    if (isDisabled(tab) || isActive(tab)) return;
    onValueChange?.(tab.name);
    onChange?.(tab.name);
  }

  async function focusTrigger(tab: ArcanaTabItem) {
