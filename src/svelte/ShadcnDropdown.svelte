<script lang="ts" module>
  ...
  export interface DropdownContextValue {
    /** Densidade herdada pelos `<ShadcnDropdownItem>` filhos. Getter — reativo. */
    readonly size: ShadcnDropdownSize;
    /** Fecha o menu (chamado pelo item ao clicar com `closeOnClick`). */
    close: () => void;
  }

  /** Chave de contexto compartilhada entre `<ShadcnDropdown>` e `<ShadcnDropdownItem>`. */
  export const DROPDOWN_CONTEXT = Symbol("shadcn-dropdown");
</script>

<script lang="ts">
  /**
   * `<ShadcnDropdown>` — Svelte 5 port do SFC Vue. Dropdown menu shadcn-style (palette
   ...
   *   substituindo o CustomEvent `shadcn-dropdown-close` do SFC (o item chama `ctx.close()`).
  ...
  const menuClasses = $derived(
    [
      ...
      size === "comfortable" ? "shadcn-dropdown__menu--comfortable" : "",
    ]
      .filter(Boolean)
      .join(" ")
  );
...
<div class="shadcn-dropdown" bind:this={containerEl}>
  <span
    class="shadcn-dropdown__trigger"
    role="presentation"
    onclick={(e) => {
      e.stopPropagation();
      toggle();
