...
<script lang="ts">
  ...
  /**
   * `<ArcanaNotice>` — Svelte 5 port. Banner informativo com variants semânticas. Reproduz
  ...
  const rootClasses = $derived(
    ["arcana-notice", `arcana-notice--${variant}`, className].filter(Boolean).join(" ")
  );
</script>
...
<div class={rootClasses} role="status">
  ...
  <div class="arcana-notice__content">
    {#if title}
      <strong class="arcana-notice__title">
        {#if typeof title === "function"}{@render title()}{:else}{title}{/if}
      </strong>
    {/if}
    <div class="arcana-notice__body">{@render children?.()}</div>
  </div>

  {#if dismissible}
    <button type="button" class="arcana-notice__close" aria-label="Fechar" onclick={() => onDismiss?.()}>
