...
<script lang="ts">
  /**
   * `<ArcanaTable>` — Svelte 5 port. Tabela estática (dados locais) shadcn. Reproduz
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  function formatCell(row: any, col: ArcanaTableColumn, index: number): any {
...
<div class={["arcana-table-wrap", className].filter(Boolean).join(" ")}>
  <table class="arcana-table">
      ...
      <tr>
        {#each columns as col (col.key)}
          <th
            ...
            class={col.align === "right" ? "arcana-table__th--right" : undefined}
          >
            {col.label}
          </th>
        {/each}
    ...
    <tbody>
      ...
      {:else}
        {#each rows as row, i (i)}
          <tr>
            {#each columns as col (col.key)}
              <td class={col.align === "right" ? "arcana-table__td--right" : undefined}>
    ...
    {#if footer}
      <tfoot class="arcana-table__foot">{@render footer()}</tfoot>
    {/if}
  </table>
</div>
