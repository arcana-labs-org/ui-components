...
/**
 * `<ArcanaNotice>` — React port. Banner informativo com variants semânticas. Reproduz
 * `<div class="arcana-notice arcana-notice--${variant}">` com `__icon`/`__content`/
...
export function ArcanaNotice({
    ...
    return (
        <div className={rootClasses} role="status">
            {showIcon ? (
                <span className="arcana-notice__icon" aria-hidden="true">
                    {iconNode ?? <i className={resolvedIcon} />}
                </span>
            ) : null}

            <div className="arcana-notice__content">
                {title ? (
                    <strong className="arcana-notice__title">{title}</strong>
                ) : null}
                <div className="arcana-notice__body">{children}</div>
            </div>

            {dismissible ? (
                <button
                    type="button"
                    className="arcana-notice__close"
                    aria-label="Fechar"
                    onClick={() => onDismiss?.()}
                >
                    <svg
