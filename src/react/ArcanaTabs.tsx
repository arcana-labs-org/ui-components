...
/**
 * `<ArcanaTabs>` — React port do SFC Vue. Reproduz o mesmo markup/classes:
 * `arcana-tabs` (+ `--${variant}`, `--${orientation}`, `--flush`), `arcana-tabs__list`,
...
export interface ArcanaTabsProps {
    ...
    renderPanel?: (tab: ArcanaTabItem) => ReactNode;
...
export function ArcanaTabs({
    ...
    const isActive = (tab: ArcanaTabItem) => tab.name === value;
    const isDisabled = (tab: ArcanaTabItem) => Boolean(tab.disabled);
    const wasActivated = (tab: ArcanaTabItem) =>
        Boolean(tab.eager) || activatedTabNames.includes(String(tab.name));
    const triggerId = (tab: ArcanaTabItem) =>
        `arcana-tabs-${uid}-trigger-${String(tab.name)}`;
    const panelId = (tab: ArcanaTabItem) =>
        `arcana-tabs-${uid}-panel-${String(tab.name)}`;
    const activeTab = normalizedTabs.find((t) => t.name === value) ?? null;

    const panelContent = (tab: ArcanaTabItem): ReactNode => {
    ...
    const select = (tab: ArcanaTabItem) => {
        if (isDisabled(tab) || isActive(tab)) return;
        onValueChange?.(tab.name);
        onChange?.(tab.name);
    };

    const focusTrigger = (tab: ArcanaTabItem) => {
    ...
    const triggerIndexOf = (tab: ArcanaTabItem) =>
        normalizedTabs.findIndex((t) => t.name === tab.name);

    return (
        <div className={["arcana-tabs", rootClasses, className ?? ""].filter(Boolean).join(" ")}>
            <div
                className="arcana-tabs__list"
                role="tablist"
            ...
            >
                ...
                {renderableItems.map((item) =>
                    item.type === "header" ? (
                        <div
                            key={item.key}
                            className="arcana-tabs__group-header"
                            role="presentation"
                        >
                            {item.label}
                        </div>
                        ...
                        >
                            {item.tab.icon ? (
                                <i
                                    className={`arcana-tabs__trigger-icon ${item.tab.icon}`}
                                    style={
                                        item.tab.iconColor
                            ...
                            <span className="arcana-tabs__trigger-label">{item.tab.label}</span>
                            {item.tab.badge != null ? (
                                <span className="arcana-tabs__trigger-badge">
                                    {item.tab.badge}
                                </span>
                            ) : null}
                        </button>
