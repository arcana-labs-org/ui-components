...
/**
 * `<ArcanaSwitchCard>` — React port. Toggle full-width de alto impacto (card inverte
...
export function ArcanaSwitchCard({
    value = false,
    title = "",
    statusOn = "ATIVO",
    statusOff = "DESLIGADO",
    ...
    return (
        ...
        >
            {hasIcon ? (
                <span className="arcana-switch-card__icon" aria-hidden="true">
                    {iconNode ?? (icon ? <i className={icon} /> : null)}
                </span>
            ) : null}

            <div className="arcana-switch-card__text">
                <div className="arcana-switch-card__title">{title}</div>
                {hasStatus ? (
                    <div className="arcana-switch-card__status">
                        {status ?? currentStatus}
                    </div>
                ) : null}
            </div>

            <span className="arcana-switch-card__switch" aria-hidden="true">
                <span className="arcana-switch-card__switch-thumb" />
            </span>
        </button>
    );
}
