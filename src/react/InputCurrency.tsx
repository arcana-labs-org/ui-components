...
/**
 * `<InputCurrency>` — React port. Input de moeda BRL.
 *
 ...
 *   centavos), mantendo markup/classes (`icur-shadcn-field`/`icur-shadcn-input` no modo
...
export function InputCurrency({
    ...
    const wrapClasses = shadcn
        ? ["icur-shadcn-field", isDisabled ? "is-disabled" : ""]
              .filter(Boolean)
              .join(" ")
        : showIcon
          ? "input-group"
        ...
        <div className={wrapClasses || undefined}>
            {prepend ?? (
                <>
                    ...
                    {showIcon && shadcn ? (
                        <span className="icur-shadcn-field__icon">
                            <i className={icon} />
                        </span>
                    ) : null}
                </>
